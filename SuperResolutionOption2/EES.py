...
def model_EES16():
    ...
    EES = Conv2DTranspose(filters=32, kernel_size=(15, 15), strides=(3, 3), padding='same', activation='relu')(EES)
...
def model_EES():
    ...
    EES = Conv2DTranspose(filters=8, kernel_size=(15, 15), strides=(3, 3), padding='same', activation='relu')(EES)
    out = Conv2D(filters=1, kernel_size=(5, 5), strides=(1, 1), activation='relu', padding='same')(EES)
...
def EES_train():
    ...
    checkpoint = ModelCheckpoint("./myTrain/EES_check.h5", monitor='val_loss', verbose=1, save_best_only=True,
    ...
    pandas.DataFrame(history_callback.history).to_csv("./myTrain/history.csv")
    EES.save_weights("./myTrain/EES_final.h5")
